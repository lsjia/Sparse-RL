...
def parse_args():
    ...
    parser.add_argument("--use_sglang", action="store_true", help="Use SGLang offline engine for inference.")
...
def setup(args):
    ...
    try:
        ...
        if args.use_sglang:
            llm = "http://localhost:30000/generate"
            print(f"Using SGLang Server at {llm}...")
        ...
        # 确保清理所有资源
        if 'llm' in locals() and args.use_sglang:
            try:
...
def main(llm, tokenizer, data_name, args):
    ...
    #print("input_prompts:",input_prompts[0].strip())
    if args.apply_chat_template:
        input_prompts = [
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_sglang:
            ...
            # Send Request (Assuming llm is the URL string)
            # Increased timeout to 600s to avoid interruptions on long generations
            try:
                response = requests.post(llm, json=payload)
                response.raise_for_status()
                sgl_outputs = response.json()
            except Exception as e:
                print(f"[Error] Failed to get response from SGLang server: {e}")
                # Optional: break or continue depending on desired fault tolerance
            ...
            # Process output
            outputs = []
            for o in sgl_outputs:
                text = o['text']
                finish_reason = o.get('meta_info', {}).get('finish_reason', None)
                outputs.append((text, finish_reason))
    ...
    if args.use_sglang:
        # [Modified] Check if llm object has shutdown method (it won't if it's a string)
        if hasattr(llm, "shutdown"):
            llm.shutdown()
