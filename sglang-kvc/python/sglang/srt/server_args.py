...
class ServerArgs:
    ...
    # [compress] algorithm options
    compress_algorithm: Optional[str] = None
    compress_max_prompt: Optional[int] = 512
    compress_max_window: Optional[int] = 8
    # only for RKV: define divide method
    compress_divide_method: str = "step_length" # "step_length", "newline"
    # only for RKV + step_length: define divide_length
    compress_divide_length: Optional[int] = 128
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        # [compress] For compression algorithm rkv and snapkv
        parser.add_argument(
            "--compress-algorithm",
            default=ServerArgs.compress_algorithm,
            help="Compress algorithms to implement self attention with compressed KV cache",
        )
        parser.add_argument(
            "--compress-max-window",
            default=ServerArgs.compress_max_window,
            help="For Compress algorithms only: max observe window size, the default value is 32.",
        )
        parser.add_argument(
            "--compress-max-prompt",
            default=ServerArgs.compress_max_prompt,
            help="For Compress algorithms only: max input prompt length, the default value is 512. If the input prompt's length is greater this, the system will call compress algorithm.",
        )
        parser.add_argument(
