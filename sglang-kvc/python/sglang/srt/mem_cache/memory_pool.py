...
class ReqToTokenPool:
    ...
    def __init__(
        ...
        # [compress] q_cache, only supports MHA at this time
        self.q_cache: Optional[List[Dict[int, Optional[torch.Tensor]]]] = None
        self.newline_compress = {}
        self.think_forbid = {}
        self.max_window: Optional[int] = None

    def init_q_cache(self, layer_num: int, max_window: int):
        self.q_cache = [
    ...
    def alloc(self, need_size: int) -> List[int]:
        ...
        if self.q_cache is not None:
            for idx in select_index:
                self.newline_compress[idx] = False
                self.think_forbid[idx] = False
                for layer in range(len(self.q_cache)):
                    self.q_cache[layer][idx] = None
    ...
    def free(self, free_index: Union[int, List[int]]):
        if isinstance(free_index, (int,)):
            ...
            if self.q_cache is not None:
                if free_index in self.newline_compress:
                    self.newline_compress.pop(free_index)
                if free_index in self.think_forbid:
                    self.think_forbid.pop(free_index)
                for layer in range(len(self.q_cache)):
                    if free_index in self.q_cache[layer]:
                        self.q_cache[layer].pop(free_index)
        else:
            self.free_slots.extend(free_index)
            if self.q_cache is not None:
                for idx in free_index:
                    if idx in self.newline_compress:
                        self.newline_compress.pop(idx)
                    if idx in self.think_forbid:
                        self.think_forbid.pop(idx)
                    for layer in range(len(self.q_cache)):
                        if idx in self.q_cache[layer]:
                            self.q_cache[layer].pop(idx)

    def clear(self):
        self.free_slots = list(range(self.size))
        if self.q_cache:
            self.q_cache = [
                {} for _ in range(len(self.q_cache))
            ]
...
class MHATokenToKVPool(KVCache):
    ...
    def _slice_kv(self, buf, selected_positions):
        """
        buf shape (common SGLang layouts):
            [num_kv_heads, total_tokens, head_dim]
        or  [total_tokens, num_kv_heads, head_dim]

        selected_positions: 1D tensor of global token indices
        """
        if selected_positions is None:
            return buf

        if selected_positions.dim() != 1:
            selected_positions = selected_positions.reshape(-1)

        # buf: [Total_Slots, Num_Heads, Head_Dim]
        return buf.index_select(0, selected_positions)


    def _get_key_buffer(self, layer_id, selected_positions=None):
        key_buf = self.k_buffer[layer_id - self.start_layer]
        if self.store_dtype != self.dtype:
            key_buf = key_buf.view(self.dtype)
        return self._slice_kv(key_buf, selected_positions)


    def get_key_buffer(self, layer_id, selected_positions=None):
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        return self._get_key_buffer(layer_id, selected_positions)


    def _get_value_buffer(self, layer_id, selected_positions=None):
        value_buf = self.v_buffer[layer_id - self.start_layer]
        if self.store_dtype != self.dtype:
            value_buf = value_buf.view(self.dtype)
        return self._slice_kv(value_buf, selected_positions)


    def get_value_buffer(self, layer_id, selected_positions=None):
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        return self._get_value_buffer(layer_id, selected_positions)


    def get_kv_buffer(self, layer_id, selected_positions=None):
        return (self.get_key_buffer(layer_id, selected_positions),
                self.get_value_buffer(layer_id, selected_positions))
