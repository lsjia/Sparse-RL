...
class FlashAttentionBackend(AttentionBackend):
    ...
    def forward_decode(
        ...
        # [compress] Define whether to use RKV compression
        use_compression = (
            hasattr(forward_batch, "retained_token_positions") 
            and forward_batch.retained_token_positions is not None
            and len(forward_batch.retained_token_positions) > 0
        ...
        if not self.use_mla:
            # Do multi-head attention
            if not use_compression: # [compress]
                key_cache, value_cache = forward_batch.token_to_kv_pool.get_kv_buffer(
                    layer.layer_id
                )
                key_cache = key_cache.view(
            ...
            if layer.is_cross_attention:
                ...
                # [compress]
                # === RKV/SnapKV (use VarLen Attention) ===
                # 1. Aggregate index and length information of the current batch
                batch_indices_list = []
                seqlens_list = []
                # for each request in the current batch
                for i in range(len(forward_batch.req_pool_indices)):
                    req_idx = forward_batch.req_pool_indices[i].item()       
                    # get the physical index retained by the request
                    if req_idx in forward_batch.retained_token_positions:
                        indices = forward_batch.retained_token_positions[req_idx]
                    else:
                        raise ValueError(f"Request {req_idx} missing retained positions")
                    
                    batch_indices_list.append(indices)
                    seqlens_list.append(len(indices))
                # 2. Construct flattened gather indices
                global_selected_indices = torch.cat(batch_indices_list).to(self.device)
                # 3. Gather KV Cache
                # shape: [Total_Retained_In_Batch, Num_Heads, Head_Dim]
                key_cache, value_cache = forward_batch.token_to_kv_pool.get_kv_buffer(
                    layer.layer_id, 
                    selected_positions=global_selected_indices
                )
                # 4. construct cu_seqlens_k (Cumulative Sequence Lengths)
                # FlashAttn VarLen 需要这个来区分扁平化 Tensor 中哪些 Token 属于哪个请求
                # shape: [0, len1, len1+len2, ..., total_len]
                cu_seqlens_k = torch.tensor(seqlens_list, device=self.device, dtype=torch.int32)
                cu_seqlens_k = torch.cumsum(cu_seqlens_k, dim=0)
                cu_seqlens_k = torch.cat([torch.tensor([0], device=self.device, dtype=torch.int32), cu_seqlens_k]).to(torch.int32)
                max_seqlen_k = max(seqlens_list)
                # 5. construct cu_seqlens_q
                batch_size = forward_batch.batch_size
                cu_seqlens_q = torch.arange(batch_size + 1, device=self.device, dtype=torch.int32)
                # 6. Call Flash Attention VarLen
                # q need to be reshaped -> [Batch_Size * 1, Num_Heads, Head_Dim]
                # key_cache / value_cache: [Total_Tokens, Num_Heads, Head_Dim]
