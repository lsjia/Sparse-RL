...
class TpModelWorker(BaseTpWorker):
    ...
    def forward_batch_generation(
        ...
        if self.model_runner.server_args.compress_algorithm is not None:
            ...
            forward_batch.compress_max_prompt = int(self.model_runner.server_args.compress_max_prompt)
            forward_batch.compress_max_window = int(self.model_runner.server_args.compress_max_window)
            forward_batch.compress_divide_length = int(self.model_runner.server_args.compress_divide_length)
            forward_batch.compress_divide_method = self.model_runner.server_args.compress_divide_method
            forward_batch.compress_out_locs = {}
            if forward_batch.compress_algorithm in ["RKV", "snapkv", "streamingllm", "h2o", "pyramidkv"]:
        ...
        if self.pp_group.is_last_rank:
            ...
            if is_verify:
                ...
                return batch_result
            # [compress]
            # --- 定义更新 Newline 状态的辅助函数 ---
            def update_newline_status(next_tokens):
                # 只有在 Decode 阶段才需要更新
                if forward_batch.forward_mode.is_decode():
                    # 确保我们有 newline_token_id 和 think_token_id
                    # 可以在 __init__ 里获取，或者这里临时获取
                    if not hasattr(self, 'newline_token_id'):
                         # 假设 model_runner 有 tokenizer
                         self.newline_token_id = [self.model_runner.tokenizer.encode("\n")[-1]]
                         # DeepSeek R1 think tokens (example)
                         self.think_token_id = [self.model_runner.tokenizer.encode("<think>")[-1], self.model_runner.tokenizer.encode("</think>")[-1]]

                    for idx, req_indice in enumerate(forward_batch.req_pool_indices):
                        req_indice = req_indice.item()
                        next_token_id = next_tokens[idx].item()
                        ...
                        # 更新 think 状态 (可选，如果你需要)
                        if next_token_id in self.think_token_id:
                            # 简单的 toggle 逻辑或者 specific token logic
                            # 原逻辑: 遇到 think token 就 forbid，否则解除？
                            # 根据原代码逻辑复刻：
            ...
            if (
                ...
                def sample_batch_func():
                    ...
                    # [compress] 在延迟采样后更新状态
                    update_newline_status(batch_result.next_token_ids)
                    return batch_result
