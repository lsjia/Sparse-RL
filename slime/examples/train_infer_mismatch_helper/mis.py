...
def compute_mis_weights(
    ...
    *,
    ...
    # [ADD]
    veto_count_tensor = torch.tensor(total_vetoed_seqs, dtype=torch.float32, device=train_log_probs[0].device)

    if torch.distributed.is_initialized():
        import torch.distributed as dist
        dist.all_reduce(veto_count_tensor, op=dist.ReduceOp.SUM)

    global_vetoed_count = int(veto_count_tensor.item())

    if torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 0 and global_vetoed_count > 0:
            wandb.log({"train/vetoed_seq_count": global_vetoed_count}, commit=False)
            
    else:
        if global_vetoed_count > 0:
            wandb.log({"train/vetoed_seq_count": global_vetoed_count}, commit=False)
